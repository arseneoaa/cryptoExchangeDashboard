...
export function getInfoFromAsksAndBids(asks: any[], bids: any[]) {
  ...
  const orderedAsks = asks
    .concat()
    .sort((order1, order2) => parseFloat(order1[0]) - parseFloat(order2[0]));
  // bids are sorted from highest to lowest
  const orderedBids = bids
    .concat()
    .sort((order1, order2) => parseFloat(order2[0]) - parseFloat(order1[0]));
  ...
  return {
    midPrice,
    spread,
    topAsks: topAsks.map((item: string[]) => ({
      price: parseFloat(item[0]),
      volume: parseFloat(item[1]),
    })),
    topBids: topBids.map((item: string[]) => ({
      price: parseFloat(item[0]),
      volume: parseFloat(item[1]),
    })),
  };
